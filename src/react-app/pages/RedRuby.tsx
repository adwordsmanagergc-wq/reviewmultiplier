...
const STORAGE_KEY = 'red_ruby_prize'

const PRIZES = [
  { number: 1, label: 'Free Spirit' },
  { number: 2, label: 'Free Double Spirit' },
  { number: 3, label: 'Bring A Friend' },
  { number: 4, label: 'Free Cocktail' },
  { number: 5, label: 'Table Half Price' },
  { number: 6, label: 'Free Spirit' },
]

const BG_IMAGE = 'https://019d4ea7-203f-7f51-8c55-110f9f85cb54.mochausercontent.com/Screenshot-2026-04-03-at-8.59.18-pm.png'

const REVIEW_URL = 'https://g.page/r/Ceguqf5pSvfgEBM/review'
...
export default function RedRuby() {
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md w-full">
        ...
        <div className="relative flex flex-col items-center mb-6">
          ...
          <button
            onClick={roll}
            disabled={isRolling}
            className="mt-6 px-10 py-3 bg-red-600 hover:bg-red-700 text-white rounded-full shadow-lg font-bold text-lg transition-all hover:scale-105 disabled:hover:scale-100 disabled:opacity-70 border-2 border-red-400"
          >
        ...
        <div className="min-h-[80px] flex items-center justify-center mb-4">
          {winner && !isRolling && !showForm && !isCheekyMessage && formSubmitted && (
            <div className="bg-gradient-to-r from-red-600 to-red-700 text-white px-5 py-3 rounded-2xl font-medium text-sm shadow-lg text-center max-w-xs">
        ...
        {showForm && winner && (
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-5 shadow-xl w-full max-w-sm mb-4">
            <div className="text-center mb-4">
              <p className="text-red-600 font-bold text-lg mb-1">🎉 You won: {winner}!</p>
              <p className="text-gray-600 text-sm">Enter your details to claim your prize and receive guest list entry</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-3">
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-red-500 focus:ring-2 focus:ring-red-200 outline-none transition-all text-gray-800"
              />
              <input
                type="email"
              ...
              <button
                type="submit"
                className="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl font-bold transition-all hover:scale-[1.02]"
              >
                Claim Prize
              </button>
            </form>
        ...
        {(formSubmitted || isCheekyMessage) && (
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-5 shadow-xl w-full max-w-sm mb-4 text-center">
            <p className="text-red-600 font-bold text-lg mb-2">🎉 You won: {winner}!</p>
            <p className="text-gray-700 mb-2">To claim your prize, leave a review and show the staff</p>
            <p className="text-gray-600 text-sm mb-4">A quick 5 star review for Ayu would be amazing. Thank you and see you at Red Ruby!</p>
            <a
              href={REVIEW_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-full font-bold shadow-lg hover:shadow-xl transition-all hover:scale-105"
            >
